...
public class E2EBusinessFlowTests
{
    ...
    public async Task E2E_HappyPath_ChatbotBillingWorkflow_Succeeds()
    {
        ...
        // ----------------------------------------------------
        var jobService = new Aivora.Services.JobService.Service(dbContext, new Aivora.Services.RealtimeService.NullRealtimeService(), Mock.Of<Aivora.Services.NotificationService.IService>());
    ...
    public async Task E2E_NegativeAndAlternativePaths_BehavesCorrectly()
    {
        ...
        await releaseBeforeApproval.Should().ThrowAsync<ValidationException>()
            .WithMessage("Milestone must be in SUBMITTED status to release remaining funds.");
