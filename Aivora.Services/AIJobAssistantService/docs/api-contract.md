...
```text
Client requirement
â†’ AI-assisted job clarification
â†’ Job publishing
â†’ Expert recommendation
â†’ Proposal submission
â†’ Proposal acceptance
â†’ Project creation
â†’ Milestone funding with simulated escrow
â†’ Deliverable submission
â†’ Client approval / revision / dispute
â†’ Payment release / refund / freeze
â†’ Review
```
...
```json
{
  "rawInput": "TÃ´i muá»‘n lÃ m chatbot tÆ° váº¥n sáº£n pháº©m cho shop má»¹ pháº©m.",
...
```json
{
  ...
  "data": {
    "job": {
      ...
      "originalDescription": "TÃ´i muá»‘n lÃ m chatbot tÆ° váº¥n sáº£n pháº©m cho shop má»¹ pháº©m.",
...
```json
{
  "rawInput": "TÃ´i lÃ m chatbot báº±ng OpenAI API, RAG, deploy web.",
...
```json
{
  "categoryId": "uuid",
  "title": "Build a Vietnamese AI Chatbot for Cosmetic Product Consultation",
  "originalDescription": "TÃ´i muá»‘n chatbot tÆ° váº¥n sáº£n pháº©m cho shop.",
...
SUBMITTED â†’ WITHDRAWN
SUBMITTED â†’ ACCEPTED
SHORTLISTED â†’ ACCEPTED
SHORTLISTED â†’ REJECTED
SHORTLISTED â†’ WITHDRAWN
```
...
```text
PENDING_PAYMENT â†’ ACTIVE
ACTIVE â†’ IN_REVIEW
ACTIVE â†’ DISPUTED
IN_REVIEW â†’ ACTIVE
IN_REVIEW â†’ COMPLETED
DISPUTED â†’ ACTIVE
ACTIVE â†’ COMPLETED
ACTIVE â†’ CANCELLED
```
...
```text
CREATED â†’ FUNDED
FUNDED â†’ IN_PROGRESS
FUNDED â†’ SUBMITTED
IN_PROGRESS â†’ SUBMITTED
SUBMITTED â†’ REVISION_REQUESTED
REVISION_REQUESTED â†’ SUBMITTED
SUBMITTED â†’ PAID
SUBMITTED â†’ DISPUTED
DISPUTED â†’ PAID
DISPUTED â†’ REFUNDED
DISPUTED â†’ REVISION_REQUESTED
```
