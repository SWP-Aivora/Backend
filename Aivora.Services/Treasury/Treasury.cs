...
public class Treasury : ITreasury
{
    ...
    public async Task ReleaseMilestoneAsync(Guid clientId, Guid milestoneId)
    {
        ...
        if (milestone.Status != MilestoneStatus.SUBMITTED && milestone.Status != MilestoneStatus.DISPUTED) throw new ValidationException("Milestone must be in SUBMITTED or DISPUTED status to be released.");
