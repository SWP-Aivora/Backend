...
7. **SignalR hub:** Chat uses `/api/v1/chat` hub (not REST), implemented in `Aivora.Services/Hubs/ChatHub.cs`. Methods: `SendMessage`, `JoinConversation`, `LeaveConversation`, `UserTyping`, `MarkAsRead`, `JoinProject`, `LeaveProject`. Events: `ReceiveMessage`, `ReadConfirmation`, `JobStatusUpdated`, `NewJobPublished`, `MilestoneUpdated`. `Error` is reserved but not currently emitted. `JoinProject`/`LeaveProject` use group name `project-{projectId}` (Client/Expert/Admin only); `RealtimeService.SendMilestoneUpdatedAsync` broadcasts `MilestoneUpdated` to that group after fund/approve/request-revision/submit-deliverable/open-dispute — it's `void` + internal fire-and-forget (`Task.Run`), not `async Task`, so it can never fail the caller's transaction.
