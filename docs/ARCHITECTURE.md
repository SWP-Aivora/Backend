...
3. **Recommendation Scoring** — the weighted scoring formula (`0.40*SkillScore + 0.20*BudgetScore + 0.20*RatingScore + 0.10*AvailabilityScore + 0.10*CompletionScore`; `PortfolioScore` is computed but hard-coded to `0` and unused) lives inline in `RecommendationService`; a Strategy-pattern extraction would make individual score components independently testable.
